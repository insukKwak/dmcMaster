...
<%@include file="/WEB-INF/views/inc/head.jsp"%>
  ...
  <body class="navbar-fixed sidebar-fixed" id="body">
	...
	function goList(){
		window.location.href = '/prdt/cust/list';
	}
    </script>
              ...
              <div class="col-12">
                <div class="card card-default">
                  ...
                  <div class="card-body">
					...
					<form name="costForm" action="/prdt/cust/save" method="post" accept-charset="utf-8">
					    <div class="row">
						<input type="hidden" name="del" value=""/>
						<input type="hidden" name="username" value="${USER.USERNAME }"/>
					    <div class="col-lg-6">
					        <div class="form-group mb-3">
					            <label for="simpleinput">거래선 구분 </label>
					            <input type="text" name="code" class="form-control" required="" value="${cost.CODE}">
					        </div>
					     </div>
					     <div class="col-lg-6">
					        <div class="form-group mb-3">
					            <label for="simpleinput">제작비 항목명</label>
					            <input type="text" name="title" class="form-control" required="" value="${cost.TITLE}">
					        </div>
					     </div>   
					    
					    <div class="col-lg-6">
					        <div class="form-group mb-3">
					            <label for="simpleinput">상위항목 </label>
					            <c:set var="upper" value="${cost.UPPER_CODE}"/>
					            <select class="form-control" name="upper_code">
					            <c:forEach var="depthList" items="${depthlist}" varStatus="idx">
									<option value="${depthList.CODE}" <c:if test="${depthList.CODE == cost.UPPER_CODE}"> selected='selected'</c:if> >${depthList.TITLE}</option>
								</c:forEach>
								</select>
					        </div>
					     </div>
					     <div class="col-lg-6">
					        <div class="form-group mb-3">
					            <label for="simpleinput">등급분류 </label>
					            <input type="text" name="ranking_code" class="form-control" required="" value="${cost.RANKING_CODE}">
					        </div>
					     </div>  	
