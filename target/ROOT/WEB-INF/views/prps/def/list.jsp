                      ...
                      <tbody>
						<c:forEach var="def" items="${defList}" varStatus="idx">
	                        <tr>
								...
								<td>${idx.count}</td>
								<td>${def.SBJT_CODE}</td>
								<td><a  href="javascript:goView('${def.SEQ}');">${def.SBJT_NAME}</a></td>
								<td>제작비</td>
								<td>${def.GRADE}</td>
								<td>${def.SEMESTER}</td>
								<td>${def.STATUS}</td>
								...
								<td>
		                            <button type="button" class="btn btn-primary" onclick="location.href='/prps/def/form?subjectcode=${def.SBJT_CODE}'">등록 </button>
		                         </td>
	                        </tr>
					  </c:forEach>  
                      </tbody>
